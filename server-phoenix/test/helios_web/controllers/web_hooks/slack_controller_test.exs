defmodule HeliosWeb.WebHooks.SlackControllerTest do
  ...
  test "check for data in table", %{conn: conn} do
    conn =
      conn
      |> put_req_header("content-type", "application/json")
      |> post(
  ...
  test "assert an author was populated in the db", %{conn: conn} do
    conn =
      conn
      |> put_req_header("content-type", "application/json")
      |> post(
    ...
    messages = get_slack_messages()
    first_message = messages |> Enum.at(0) |> IO.inspect()
    assert Map.fetch(first_message, :source_channel) == {:ok, "CXXXXXXXX"}
  end
