defmodule Helios.TorchContextTest do
  ...
  @valid_attrs %{id: 42, inserted_at: ~N[2022-06-29 18:43:00], public_key: "some public_key", updated_at: ~N[2022-06-29 18:43:00], user_name: "some user_name"}
  @update_attrs %{id: 43, inserted_at: ~N[2022-06-30 18:43:00], public_key: "some updated public_key", updated_at: ~N[2022-06-30 18:43:00], user_name: "some updated user_name"}
  @invalid_attrs %{id: nil, inserted_at: nil, public_key: nil, updated_at: nil, user_name: nil}
  ...
  describe "#create_user/1" do
    test "with valid data creates a user" do
      assert {:ok, %User{} = user} = TorchContext.create_user(@valid_attrs)
      assert user.id == 42
      assert user.inserted_at == ~N[2022-06-29 18:43:00]
      assert user.public_key == "some public_key"
      assert user.updated_at == ~N[2022-06-29 18:43:00]
      assert user.user_name == "some user_name"
    end
  ...
  describe "#update_user/2" do
    test "with valid data updates the user" do
      ...
      assert %User{} = user
      assert user.id == 43
      assert user.inserted_at == ~N[2022-06-30 18:43:00]
      assert user.public_key == "some updated public_key"
      assert user.updated_at == ~N[2022-06-30 18:43:00]
      assert user.user_name == "some updated user_name"
    end
